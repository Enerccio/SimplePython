...
/**
 * Java method or function callable. If called is null, it acts as static method
 * callable.
 * 
 * @author Enerccio
 *
...
public class JavaMethodObject extends CallableObject {
	...
	/** Internal use */
	private JavaMethodObject(Object caller, Method m, String[] argNames,
			String pydoc, boolean noTypeConversion) {
		this.caller = caller;
		this.boundHandle = m;
		...
		if (pydoc != null)
			this.fields.put(__DOC__, new AugumentedPythonObject(
					new StringObject(pydoc), null));
	}
	...
	public JavaMethodObject(Object caller, Method m) {
		...
		this.boundHandle = m;
		this.boundHandle.setAccessible(true);
		this.argNames = m.isAnnotationPresent(ArgNames.class) ? m
				.getAnnotation(ArgNames.class).value() : null;
		this.noTypeConversion = false;
		if (m.isAnnotationPresent(SpyDoc.class))
			this.fields.put(__DOC__, new AugumentedPythonObject(
					new StringObject(m.getAnnotation(SpyDoc.class).value()),
					null));
	}

	/**
	 * Creates new Java Method Object for static method that uses specified
	 * signature, with type conversion.
	 * 
	 * @param caller
	 *            object this method is bound to
	 * @param name
	 *            method name
	 */
	public JavaMethodObject(Class<?> cls, String name, Class<?>... tp)
			throws NoSuchMethodException, SecurityException {
		this(null, cls.getMethod(name, tp), cls.getMethod(name, tp)
				.isAnnotationPresent(ArgNames.class) ? cls.getMethod(name, tp)
				.getAnnotation(ArgNames.class).value() : null, getPydoc(cls
				.getMethod(name, tp)), false);
	}

	/**
	 * Creates new Java Method Object for method that uses specified signature,
	 ...
	 *            method name
	 */
	public JavaMethodObject(Object caller, String name, Class<?>... tp)
			throws NoSuchMethodException, SecurityException {
		this(caller, caller.getClass().getMethod(name, tp),
				caller.getClass().getMethod(name, tp)
						.isAnnotationPresent(ArgNames.class) ? caller
						.getClass().getMethod(name, tp)
						.getAnnotation(ArgNames.class).value() : null,
				getPydoc(caller.getClass().getMethod(name, tp)), false);
	}

	/**
	 * Creates new Java Method Object for method that uses default signature -
	 * method(TupleObject, KwArgs), without type conversion. This constructor
	 * ignores @ArgNames annotation.
	 * 
	 * @param caller
	 *            object this method is bound to
	 * @param name
	 *            method name
	 */
	public JavaMethodObject(Object caller, String name)
			throws NoSuchMethodException, SecurityException {
		this(caller, caller.getClass().getMethod(name,
				new Class<?>[] { TupleObject.class, KwArgs.class }), null,
				getPydoc(caller.getClass().getMethod(name,
						new Class<?>[] { TupleObject.class, KwArgs.class })),
				true);
	}
	 ...
	 *            method name
	 */
	public JavaMethodObject(Class<?> cls, String name)
			throws NoSuchMethodException, SecurityException {
		this(null, cls.getMethod(name, new Class<?>[] { TupleObject.class,
				KwArgs.class }), null, getPydoc(cls.getMethod(name,
				new Class<?>[] { TupleObject.class, KwArgs.class })), true);
	}

	public static JavaMethodObject noArgMethod(Class<?> cls, String name)
			throws NoSuchMethodException, SecurityException {
		return new JavaMethodObject(cls, name, new Class<?>[] {});
	}

	public static JavaMethodObject noArgMethod(Object o, String name)
			throws NoSuchMethodException, SecurityException {
	...
	private static String getPydoc(Method m) {
		return m.isAnnotationPresent(SpyDoc.class) ? m.getAnnotation(
				SpyDoc.class).value() : null;
	}
	 ...
	 * @return
	 */
	public PythonObject doCall(TupleObject args, KwArgs kwargs)
			throws PointerMethodIncompatibleException {
		try {
			if (noTypeConversion) {
				return Coerce.toPython(invoke(args, kwargs),
						boundHandle.getReturnType());
			}
		...
		try {
			...
			while (i < args.len()) {
				if ((i >= jargs.length) || (argTypes[i] == KwArgs.class))
					throw new PointerMethodIncompatibleException(toString()
							+ " doesn't take " + args.len() + " arguments");
				jargs[i] = Coerce.toJava(args.get(i), argTypes[i]);
				if ((kwargs != null) && (argNames != null)
						&& kwargs.contains(argNames[i]))
					throw new PointerMethodIncompatibleException(toString()
			...
			if ((argNames != null) && (kwargs != null)) {
				while (i < argTypes.length) {
					...
					if (kwargs.contains(argNames[i])) {
						jargs[i] = Coerce.toJava(kwargs.consume(argNames[i]),
								argTypes[i]);
					} else {
						throw new PointerMethodIncompatibleException(toString()
								+ " values for argument '" + argNames[i]
								+ "' missing");
					}
					i++;
				}
			...
			if ((argNames == null) || (i >= argNames.length))
				throw new PointerMethodIncompatibleException(toString()
						+ " cannot convert value for argument " + i);
			else
				throw new PointerMethodIncompatibleException(toString()
						+ " cannot convert value for argument '" + argNames[i]
						+ "'", e);
		}
	...
	protected String doToString() {
		...
		return "<java method " + boundHandle.getName() + " of object "
	...
	public JavaMethodObject cloneWithThis(Object self) {
		return new JavaMethodObject(
				self,
				boundHandle,
				argNames,
				this.fields.containsKey(__DOC__) ? this.fields.get(__DOC__).object
						.toString() : null, noTypeConversion);
	}
