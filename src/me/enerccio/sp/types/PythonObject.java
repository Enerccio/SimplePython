...
public abstract class PythonObject implements Externalizable, HasEqMethod,
	...
	static {
		try {
			sfields.put(__EQ__, new JavaMethodObject(PythonObject.class, "eq",
					PythonObject.class));
			sfields.put(__NE__, new JavaMethodObject(PythonObject.class, "ne",
					PythonObject.class));
			sfields.put(__FORMAT__, new JavaMethodObject(PythonObject.class,
					"format", String.class));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void bindMethod(String name, JavaMethodObject m) {
		fields.put(name, new AugumentedPythonObject(m.cloneWithThis(this),
	 ...
	 */
	public Map<String, AugumentedPythonObject> fields = Collections
			.synchronizedMap(new HashMap<String, AugumentedPythonObject>(16));

	public Map<String, AugumentedPythonObject> getEditableFields() {
		return fields;
	}

	private Map<String, SoftReference<JavaMethodObject>> genFields = Collections
			.synchronizedMap(new HashMap<String, SoftReference<JavaMethodObject>>(
					16));
	...
	public synchronized PythonObject get(String key, PythonObject localContext) {
		...
		if (field.restrictions == AccessRestrictions.PRIVATE
				&& !isPrivate(localContext, field))
			throw new AttributeError("access to field '" + key
					+ "' is restricted for type '"
	...
	public synchronized void create(String key,
			AccessRestrictions restrictions, PythonObject currentContext) {
		if (fields.containsKey(key))
			throw new AttributeError("'"
					+ Utils.run("str", Utils.run("typename", this))
					+ "' object already has a attribute '" + key + "'");
		AugumentedPythonObject field = new AugumentedPythonObject(
				NoneObject.NONE, restrictions, currentContext);
